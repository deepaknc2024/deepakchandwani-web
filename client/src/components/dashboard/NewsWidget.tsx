...
function NewsCard({ item, flag }: { item: NewsItem; flag: string }) {
  ...
  return (
    ...
    >
      <div className="mb-1 text-[0.58rem] font-bold uppercase tracking-widest text-cyan-2">
        {flag}
      </div>
      <div className="line-clamp-3 text-[0.78rem] font-semibold leading-snug text-ink">
        {item.title}
      </div>
...
export default function NewsWidget() {
  ...
  const flag = activeTab === "us" ? "\ud83c\uddfa\ud83c\uddf8 USA" : "\ud83c\uddee\ud83c\uddf3 India";
        ...
        <div className="flex gap-1">
          <button
            ...
            className={`rounded-lg border px-2.5 py-0.5 text-[0.65rem] font-semibold transition-all ${
              activeTab === "us"
                ? "border-[#a5f3fc] bg-[#ecfeff] text-cyan-2"
                : "border-bdl bg-white text-muted hover:text-body"
            }`}
          >
            USA
          </button>
      ...
      {loading ? (
        <div className="grid grid-cols-4 gap-2">
          {Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="h-22 animate-pulse rounded-xl bg-light-2 border border-bdl"
            />
          ))}
        </div>
      ) : items.length === 0 ? (
        ...
        >
          <div className="flex animate-[tick_55s_linear_infinite] gap-10 whitespace-nowrap hover:[animation-play-state:paused]">
            {tickerItems.map((item, i) => (
              ...
              >
                <span className="text-[0.56rem] text-cyan-2">
                  {"\u25b8"}
                </span>
                {item.title}
              </span>
